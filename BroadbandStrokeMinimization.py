...
class BroadbandStrokeMinimization(StrokeMinimization):
    ...
    def take_exposure(self, devices, exposure_type, wavelength, initial_path, flux_attenuation_factor=1., suffix=None,
        ...
        latest_dir = max(glob.glob(os.path.join(initial_path, '*/')), key=os.path.getmtime)
        for processed_im in ['*cal.fits', '*bin.fits']:
            file_path = glob.glob(os.path.join(latest_dir, '*{}'.format(exposure_type), '*{}'.format(exposure_type), processed_im))[0]
            fits.setval(filename=file_path, keyword='ATTENFAC', value=flux_attenuation_factor)
...
def main():
    """
    Configurations for various control modes:

    1. Monochromatic imaging with MCLS1
        - Change testbed/laser source in config.ini to thorlabs_source_mcls1.  The control algorithm will override the
        wavelength and weight parameters below and just use 638 nm.
        - You still need to plug in 638. for the wavelength so that the Jacobian can be read.

    For the other cases, testbed/laser source in config.ini should be set to light_source_assembly:

    2. Broadband imaging over 6% bandpass with broadband control (equal weighting)
        - wavelengths = [640., 620., 660.]  (the center wavelength MUST be first)
        - control_weights = [1, 1, 1]

    2. Broadband imaging over 6% bandpass, but only controlling center wavelength
        - wavelengths = [640., 620., 660.]
        - control_weights = [1, 0, 0]

    3. Monochromatic imaging with monochromatic control (using Thorlabs source)
        - wavelengths = [640.]
        - control_weights = [1]

    The specific wavelength and weight combinations can be adjusted as needed.  Always ensure that control_weights has
    the same number of elements as wavelengths.  The control weights have the same ordering as the wavelengths, so
    wavelengths[0] -> control_weights[0], wavelengths[1] -> control_weights[1], and so on.  Remember that the center
    wavelength must always be the first in the list.
    """
    gamma = 0.9           # Stroke min decrease per step
    auto_adjust_gamma = True   # Start gamma at 0.5 then gradually approach the above value?
    num_iterations = 80   # Number of iterations of Stroke Min before script terminates
    num_exposures = 20    # Number of exposures to take per each probe
    exposure_time_coron = 50000  # Exposure time for coronagraphic images, microseconds
    exposure_time_direct = 2000  # Exposure time for direct images, microseconds
    use_dm2 = True
    direct_every = 1        # Take direct image every 5th iteration
    resume = False  # Resume from last dark zone DM settings?
    dm_command_dir_to_restore = 'None'  # option to restore to a particular directory
    autoscale_e_field = True  # Automatically determine "fudge factor" each iteration?
    control_gain = 0.5
    mu_start = 1e-7  # Starting guess for stroke minimization regularization parameter
    wavelengths = [610., 620., 640., 660., 670.]  # Control wavelengths, in nm.  The center wavelength must be first.
    num_wavelengths = len(wavelengths)

    # Select filters for direct and coronagraphic mode.
    nd_direct = num_wavelengths * ['9_percent']
    nd_coron = num_wavelengths * ['clear_1']

    # Relative flux at each wavelength.
    # TODO: this should be removed in the future since we separately normalize each wavelength. Leave as-is for now.
    spectral_weights = num_wavelengths * [1]

    # How much each wavelength is weighted in the control problem.  This list needs to have (num_wavelength) elements.
    control_weights = [1, 1, 1, 1, 1]
    perfect_knowledge_mode = False  # Use perfect knowledge of E-field? (only works in simulation mode)

    jacobian_path = observation_matrix_path = hicat.util.map_jacobian_path()

    # Universal matrix names
    jacobian_names = [f'2020-03-08_CLC_BB_5lam/jacobian_2020-03-08_no_wfe_broadband_test_5lam_clc_{int(np.rint(wl))}nm.fits' for wl in wavelengths]
    observation_matrix_names = [f'2020-03-08_CLC_BB_5lam/probes_2020-03-09_clc_with_wfe_broadband_jacobian_{int(np.rint(wl))}nm_dz=6-11_rcond_est=0.1_rcond_probe=0.7_num_probes=4_tilt_x=0_tilt_y=0_ptv=15_fullDZcontrol.fits' for wl in wavelengths]

    # Concatenate and run
    jacobian_filenames = [os.path.join(jacobian_path, filename) for filename in jacobian_names]
    observation_matrix_filenames = [os.path.join(observation_matrix_path, filename) for filename in observation_matrix_names]

    for jacobian, obs_mat in zip(jacobian_filenames, observation_matrix_filenames):
        print('Jacobian filenames: {}'.format(jacobian))
        print('Observation matrix filename: {}\n'.format(obs_mat))

    if sim:
        num_exposures = 1

    print('Starting stroke minimization algorithm...')
    BroadbandStrokeMinimization(
        wavelengths=wavelengths,
        jacobian_filenames=jacobian_filenames,
        probe_filenames=observation_matrix_filenames,
        num_iterations=num_iterations,
        num_exposures=num_exposures,
        exposure_time_coron=exposure_time_coron,
        exposure_time_direct=exposure_time_direct,
        use_dm2=use_dm2,
        gamma=gamma,
        control_gain=control_gain,
        auto_adjust_gamma=auto_adjust_gamma,
        direct_every=direct_every,
        autoscale_e_field=autoscale_e_field,
        resume=resume,
        dm_command_dir_to_restore=dm_command_dir_to_restore,
        mu_start=mu_start,
        nd_direct=nd_direct,
        nd_coron=nd_coron,
        control_weights=control_weights,
        spectral_weights=spectral_weights,
        perfect_knowledge_mode=perfect_knowledge_mode
    ).start()
