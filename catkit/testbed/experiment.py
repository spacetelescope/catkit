...
class Experiment(ABC):
    ...
    def __init__(self, safety_tests, safety_check_interval=60, output_path=None, suffix=None):
        ...
        self.safety_tests = []
        self.safety_tests.extend(safety_tests)
        ...
        try:
            ...
            for safety_test in self.safety_tests:
                ...
                if not status:
                    ...
                    raise SafetyTest.Exception(errmessage)
            ...
            while experiment_process.is_alive():
                ...
                if not self.smart_sleep(self.safety_check_interval, experiment_process):
            ...
            raise
        except SafetyTest.Exception:
            self.log.exception("Safety exception.")
            raise
        except Exception as e:
            safety_exception = SafetyTest.Exception("Monitoring process caught an unexpected problem: ", e)
            self.log.exception(safety_exception)
            # Shut down the experiment (but allow context managers to exit properly).
            if experiment_process is not None:
                self.soft_kill()
            # must return SafetyTest.Exception type specifically to signal queue to stop in typical calling scripts
    ...
    def soft_kill(self):
        ...
        if os.name == "nt":
            ...
            try:
                log.info("Sending ctrl-c event...")
                os.kill(experiment_process.pid, signal.SIGINT)
    ...
    @staticmethod
    def smart_sleep(interval, process):
        """
