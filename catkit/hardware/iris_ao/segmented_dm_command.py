...
class SegmentedDmCommand(object):
    ...
    def __init__(self, data=None, flat_map=False, dm_mapping=None, config_id='iris_ao'):
        ...
        :param dm_mapping: bool or None, whether or not the input command is in the Iris
                           frame already.
        """
        ...
        try:
            ...
            if (self.segments_in_pupil[0] != 1) and (not dm_mapping):
...
def display(segment_values, instrument_fov, active_segment_list=None, iris_mapping=None,
    ...
    data, dm_mapping = iris_util.read_segment_values(segment_values, iris_mapping)
    # Shift TO Poppy from the Iris AO
    if not active_segment_list or not dm_mapping:
