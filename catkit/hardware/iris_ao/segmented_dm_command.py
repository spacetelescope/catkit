...
class SegmentedDmCommand(object):
    ...
    def add_map(self, segment_values_to_add, flat=False):
        ...
        data_to_add, _ = util.read_segment_values(segment_values_to_add)
...
def shift_command(command_to_shift, to_pupil, from_pupil=None):
    ...
    shifted_map = util.remap_dictionary(command_to_shift, mapping)
...
def load_command(segment_values, flat_map=True):
    ...
    :param segment_values: str, list, np.ndarray, dict. Can be .PTT111, .ini files,
    ...
    """
    data, source_pupil_numbering = util.read_segment_values(segment_values)
    return SegmentedDmCommand(data, flat_map=flat_map,
                              source_pupil_numbering=source_pupil_numbering)
