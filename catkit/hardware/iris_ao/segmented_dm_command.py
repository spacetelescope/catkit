...
class SegmentedDmCommand(object):
    ...
    """
    def __init__(self, data=None, flat_map=False, native_mapping=None, config_id='iris_ao'):
        """
        ...
        try:
            ...
            if (self.segments_in_pupil[0] != 1) and (not native_mapping):
...
def load_command(segment_values, flat_map=True):
    ...
    """
    data, native_mapping = iris_util.read_segment_values(segment_values)
    return SegmentedDmCommand(data, flat_map=flat_map, native_mapping=native_mapping)
...
def display(segment_values, instrument_fov, active_segment_list=None, dm_mapping=None,
    ...
    :param dm_mapping: bool or None, whether or not the input command is in the Native
                       numbering frame (Iris frame) already. If None, this will be
                       determined by input type of segment_values. If segment_values
                       is a .PTT111 or .ini file, assumes dm_mapping=True. If segment_values
                       is a dictionary, assumes dm_mapping=False.
    ...
    data, native_mapping = iris_util.read_segment_values(segment_values, dm_mapping)
    print(native_mapping)
    # Shift TO Poppy from the Iris AO
    if not active_segment_list or not native_mapping:
    ...
    iris = deploy_global_wf(iris, command) # takes pupil and dictionary
